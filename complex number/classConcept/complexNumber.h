...
complexNumber complexNumber::addComplex(const complexNumber& oth) { // const tanýmladýk çünkü
	complexNumber result;											// içerisindeki bilgiyi kullanýyoduk
	result.real = real + oth.real;
	result.imag = imag + oth.imag;
	return result; // return iþleminde geçici bir copy constractur çalýþýyo ve destructor ediliyo.
}

complexNumber complexNumber::subtractComplex(complexNumber& oth) { // bunda sadece içersiindekini kullanmicaz
...
complexNumber complexNumber::multiplyComplex(const complexNumber& oth) {
	...
	result.imag = real * oth.real + imag * oth.imag;
	return result;
}